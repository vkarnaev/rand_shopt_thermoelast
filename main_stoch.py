...
ind = 1
for s in range(1, NSPACEEIGENS+1):
    for t in range(1, NTIMEEIGENS+1):
        mechtools.thermal_stoch(path.step(0,"mesh"), s, t, ind)
        mechtools.thermoelastic_stoch(path.step(0,"mesh"), ind)
        ind+=1
  ...
  # Calculation of the objective and constraint functions
    def evalObjective(self,x) :
        ...
        if not self.ucomputed :
            ...
            for s in range(1, NSPACEEIGENS+1):
                for t in range(1, NTIMEEIGENS+1):
                    mechtools.thermal_stoch(x, s, t, ind)
                    mechtools.thermoelastic_stoch(x, ind)
                    ind+=1
    ...
    def evalSensitivities(self,x,**kwargs) :
        ...
        if not self.sensitivity_computed :
            ...
            if len(ineq_indices) > 0:
                for ind in range(NEIGENS+1):
                    mechtools.adjoints_stoch(x,ind)
