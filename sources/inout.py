...
def iniWF():
  ...
  # Create and clear ./plot folder for results depending on the situation
  proc = subprocess.Popen(["mkdir -p {folder}".format(folder=path.PLOTDIR)],shell=True)
  proc.wait()
  proc = subprocess.Popen(["rm -rf {folder}*".format(folder=path.PLOTDIR)],shell=True)
  proc.wait()
  ...
  setAtt(file=path.EXCHFILE,attname="Fourier",attval=path.REFFOU)
  ...
  #setAtt(file=path.EXCHFILE,attname="CoefficientConstraint",attval=path.AC)
  ...
  setAtt(file=path.EXCHFILE,attname="LameLambda",attval=path.LAMEL)
  setAtt(file=path.EXCHFILE,attname="LameMu",attval=path.LAMEM)
  setAtt(file=path.EXCHFILE,attname="ThermalK",attval=path.THERMK)
  setAtt(file=path.EXCHFILE,attname="ThermalBeta",attval=path.THERMBETA)
  setAtt(file=path.EXCHFILE,attname="ThermalQgen",attval=path.THERMQGEN)
  setAtt(file=path.EXCHFILE,attname="TemperatureDirichlet",attval=path.THERMDIR)
  setAtt(file=path.EXCHFILE,attname="ThermalCoupling",attval=path.ALPHATHERM)
  setAtt(file=path.EXCHFILE,attname="Loadx",attval=path.GX)
  setAtt(file=path.EXCHFILE,attname="Loady",attval=path.GY)
  setAtt(file=path.EXCHFILE,attname="DensityOfMaterial",attval=path.DENSITY)
  setAtt(file=path.EXCHFILE,attname="NumberOfTimesteps",attval=path.NTIMESTEPS)
  setAtt(file=path.EXCHFILE,attname="DeltaTime",attval=path.DELTATIME)
  setAtt(file=path.EXCHFILE,attname="DeltaTemperatureWave",attval=path.THERMWAVE)
  
  setAtt(file=path.EXCHFILE,attname="TemperatureName",attval=path.TEMPERATUREPATTERN)
  setAtt(file=path.EXCHFILE,attname="TemperatureExtension",attval=path.TEMPERATUREEXTENSION)
  setAtt(file=path.EXCHFILE,attname="DisplName",attval=path.DISPLPATTERN)
  setAtt(file=path.EXCHFILE,attname="DisplExtension",attval=path.DISPLEXTENSION)
  setAtt(file=path.EXCHFILE,attname="AdjointTemperatureName",attval=path.ADJOINTTEMPERATUREPATTERN)
  setAtt(file=path.EXCHFILE,attname="AdjointTemperatureExtension",attval=path.ADJOINTTEMPERATUREEXTENSION)
  setAtt(file=path.EXCHFILE,attname="AdjointDisplacementName",attval=path.ADJOINTDISPLACEMENTPATTERN)
  setAtt(file=path.EXCHFILE,attname="AdjointDisplacementExtension",attval=path.ADJOINTDISPLACEMENTEXTENSION)
  setAtt(file=path.EXCHFILE,attname="WeightOfAverage",attval=path.WEIGHTAVG)

  setAtt(file=path.EXCHFILE,attname="KernelParam",attval=path.KERNELPARAM)
...
######                    J   (real) value of volume                       ######
######                    H  (list) table of constraints                   ######
#################################################################################

def printHisto(it,J,H):

  histo = open(path.HISTO,'a')
  print("{} {} ".format(it,J) + " ".join(map(str,H)),file=histo)
  histo.close()
