...
def norvec(mesh,phi,v,V) :
  ...
  if v is None:
    inout.setAtt(file=path.EXCHFILE,attname="SolName",attval="-")
  else:
...
##################################################################################################
#############             Enforcement of the non-optimizable regions                 #############
##################################################################################################

def enforceNonOptimizable(curmesh, curphi, newphi):
  inout.setAtt(file=path.EXCHFILE,attname="MeshName",attval=curmesh)
  inout.setAtt(file=path.EXCHFILE,attname="PhiName",attval=curphi)
  inout.setAtt(file=path.EXCHFILE,attname="NewPhiName",attval=newphi)
  
  execFreeFem.executeFF(nameScript=path.FFNONOPT, errorMessage = "Error in enforcement of the non-optimizable regions to the level-set; abort.")
